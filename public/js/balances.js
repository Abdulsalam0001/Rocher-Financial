...
const effectiveAt=document.querySelector("#effectiveAt");
if(effectiveAt)effectiveAt.value=localDateTime();
...
m.textContent=r.ok?(body.effectiveAt?"Historical adjustment posted. Reference: ":"Adjustment posted. Reference: ")+j.reference:j.error||"Unable to post adjustment.";if(r.ok){e.currentTarget.reset();if(effectiveAt)effectiveAt.value=localDateTime();await load()}});
