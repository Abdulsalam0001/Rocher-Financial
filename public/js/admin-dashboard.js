...
async function load(){const results=await Promise.all([get("/api/admin/overview"),get("/api/admin/customers"),get("/api/admin/history")]);const o=results[0],c=results[1],h=results[2];if(!o)return;document.querySelector("#customersCount").textContent=o.customers;document.querySelector("#accountsCount").textContent=o.accounts;document.querySelector("#transactionsCount").textContent=o.transactions;document.querySelector("#auditCount").textContent=o.auditLogs;document.querySelector("#customers").innerHTML=(c||[]).map(x=>{const a=x.accounts[0];return "<tr><td><strong>"+x.name+"</strong></td><td>"+x.email+"</td><td>"+(a?a.accountNumber:"—")+"</td><td>"+(a?money(a.balanceMinor,a.currency):"—")+"</td><td class='status'>"+x.status+"</td><td><div class='customer-actions'><button class='button small change-password' type='button' data-customer-id='"+x.id+"' data-customer-name='"+x.name+"'>Change password</button><button class='delete-customer' type='button' data-customer-id='"+x.id+"' data-customer-name='"+x.name+"'>Delete user</button></div></td></tr>"}).join("")||"<tr><td colspan='6'>No customers.</td></tr>";const items=[];(h?.transactions||[]).forEach(x=>items.push({date:x.createdAt,title:"Transaction "+x.reference,detail:x.type+" · "+money(x.amountMinor,x.currency)+" · "+x.status}));(h?.auditLogs||[]).forEach(x=>items.push({date:x.createdAt,title:x.action,detail:x.resource+(x.user?.email?" · "+x.user.email:"")}));(h?.securityEvents||[]).forEach(x=>items.push({date:x.createdAt,title:x.event,detail:x.user?.email||"System"}));items.sort((a,b)=>new Date(b.date)-new Date(a.date));document.querySelector("#historyList").innerHTML=items.slice(0,30).map(x=>"<div class='history-item'><strong>"+x.title+"</strong> — "+x.detail+"<br><small>"+new Date(x.date).toLocaleString()+"</small></div>").join("")||"<p>No activity yet.</p>"}
async function loadContactMessages(){
 const body=document.querySelector("#contactMessages"); if(!body)return;
 try{const r=await fetch("/api/admin/contact-messages");if(r.status===401){location.href="/admin/login.html";return}const rows=await r.json();body.innerHTML=(rows||[]).map(x=>"<tr><td>"+new Date(x.createdAt).toLocaleString()+"</td><td><strong>"+escapeHtml(x.name)+"</strong><br><a href='mailto:"+escapeHtml(x.email)+"'>"+escapeHtml(x.email)+"</a></td><td>"+escapeHtml(x.subject)+"</td><td style='white-space:pre-wrap'>"+escapeHtml(x.message)+"</td></tr>").join("")||"<tr><td colspan='4'>No contact messages yet.</td></tr>";}
 catch(e){body.innerHTML="<tr><td colspan='4'>Unable to load messages.</td></tr>"}
}
function escapeHtml(v){return String(v??"").replace(/[&<>"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[c]))}
document.querySelector("#refreshMessages")?.addEventListener("click",loadContactMessages);
document.querySelector("#customers").addEventListener("click",async e=>{
 const b=e.target.closest(".delete-customer");if(!b)return;
 const confirmed=window.Swal?await Swal.fire({title:"Delete this user?",text:"This permanently removes the customer and their account records.",icon:"warning",showCancelButton:true,confirmButtonText:"Delete user",confirmButtonColor:"#b42318"}).then(x=>x.isConfirmed):confirm("Permanently delete "+b.dataset.customerName+" and their account records?");
 if(!confirmed)return;
 b.disabled=true;
 try{const r=await fetch("/api/admin/customers/"+encodeURIComponent(b.dataset.customerId),{method:"DELETE"});const j=await r.json();if(!r.ok)throw new Error(j.error||"Unable to delete user.");RMAlert?.success("User deleted","The customer and related account records were removed.");await load();}
 catch(err){RMAlert?.error("Delete failed",err.message||"Unable to delete user.");b.disabled=false}
});
